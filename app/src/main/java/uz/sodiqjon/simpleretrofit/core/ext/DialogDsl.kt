...
//fun Activity.showAlert(title: Int = R.string.error_title, init: AlertDialog.Builder.() -> Unit) =
//    alert {
//        setTitle(title)
//        init()
//    }.show()
...
fun AlertDialog.Builder.okButton(ok: String, action: (() -> Unit)? = null) =
    apply { setPositiveButton(ok) { _, _ -> action?.invoke() } }

fun AlertDialog.Builder.okButton(
    @StringRes ok: Int = android.R.string.ok,
    action: (() -> Unit)? = null
) = apply { setPositiveButton(ok) { _, _ -> action?.invoke() } }

fun AlertDialog.Builder.cancelButton(cancel: String, action: (() -> Unit)? = null) =
    apply { setNegativeButton(cancel) { _, _ -> action?.invoke() } }

fun AlertDialog.Builder.cancelButton(
    @StringRes cancel: Int = android.R.string.cancel,
    action: (() -> Unit)? = null
) = apply { setNegativeButton(cancel) { _, _ -> action?.invoke() } }
...
fun AlertDialog.Builder.iconVector(@DrawableRes icon: Int) =
    apply { setIcon(this.context.getDrawableFromVector(icon)) }
